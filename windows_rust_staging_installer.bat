...
SET "ROOT_DIR=C:\rustserverstaging"
:: -------------------------------
...
SET "OXIDE_URL=https://downloads.oxidemod.com/artifacts/Oxide.Rust/staging/Oxide.Rust.zip"
...
echo  2) Install/Update Rust dedicated server (AppID 258550 - STAGING)
echo  3) Install/Update Oxide (uMod - staging)
...
pushd "%SteamCmdPath%" >nul
start "" /wait "%SteamCmdExe%" +force_install_dir "%ServerPath%" +login anonymous +app_update 258550  -beta staging validate +quit
